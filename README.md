...
In 2026, I had ChatGPT look at my code. It found this paper: [https://arxiv.org/abs/1803.06816](https://arxiv.org/abs/1803.06816), which includes my swap-counting problem as a special case! ChatGPT confirmed my proofs above and explained them to me in fancy terms of *multisets* and *Eulerian directed multigraphs*. It then helped me implement some of my ideas: it coded swapForcedSameSource() for me and edited findCyclesToGetMinSwaps() to make *cycles_seen* a set. I implemented these changes in waffle.py and waffleGen2.py.
...
In 2026, ChatGPT made waffleGen.py (and solidWaffleGen.py) about 60 times faster by precomputing dictionaries of words indexed by their required shared letters!
...
* If a shared letter is yellow and appears once in both words' solution, a duplicate of that letter wasn't yellow if in one of the words, but was yellow if in the other word. This might be considered a bug because the letter should could toward both words.
...
Next steps...
* Prove whether or not a simple pre-determined coloring order (without exploring all permutations) for shared locations that remove from two words exists to make the puzzle have the fewest yellows. If so, consider coding it! If not, consider coding the permutations! Regardless, each letter can be done separately. ChatGPT in 2026 says: "You may not need to explore all permutations. For each letter, the 'fewest yellows' version can probably be solved as a small bipartite matching / flow problem, where horizontal words and vertical words are nodes, and shared candidate cells are edges."
