...
* One way to speed up the now-commented-out permutation code is to remove permutations that are the same. For example, if AABBCC is the puzzle with solution BBCCAA, my code would check 8 permutations, but there really is only 1 that is needed. Since runtime depends on the size of the problem factorially, this, in certain situations, could very much help. I bet the following would be helpful: [https://stackoverflow.com/a/72969842](https://stackoverflow.com/a/72969842)
...
* Currently, if the words THESE and THEME were in the same puzzle, with the M and S unsolved for and the M or S start not in one of these words as a yellow letter, my code would say that there are at least 2 solutions, and it would reject the puzzle. However, I wonder if it could still be possible for the user to do the optimal swaps in a way that allows them to figure out which word is which.
