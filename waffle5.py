...
#
# Find optimal (fewest swap) solutions for Waffle puzzles...
#   https://wafflegame.net/daily
#
# (c) 2023 Bradley Knockel

# For numbering the 6 words...
#   .0.0.
#   3 4 5
#   .1.1.
#   3 4 5
...
for wordNum in range(nl+1):
    ...
    for word in freq:
      ...
      if go:
        wordList.append((freq[word], word))
    ...
    for entry in sorted(wordList, reverse=True):
...
def loop_recursive(w, n):
  ...
  if n <= nl:   # there are nl+1 words
      ...
      # check waffle shape
      if w[0][0::2] != w[3][0] + w[4][0] + w[5][0]:
        return
      if w[1][0::2] != w[3][2] + w[4][2] + w[5][2]:
        return
      if w[2][0::2] != w[3][4] + w[4][4] + w[5][4]:
        return

      # check counts
      letters = w[0] + w[1] + w[2] + w[3][1::2] + w[4][1::2] + w[5][1::2]
      ...
      print()
      print(" ", w[0])
      print(" ", w[3][1] + " " + w[4][1] + " " + w[5][1])
      print(" ", w[1])
      print(" ", w[3][3] + " " + w[4][3] + " " + w[5][3])
      print(" ", w[2])
